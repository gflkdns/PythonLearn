        ...
        for i in indices:
            ...
            if normpath != norminfile or not skipOriginalMorph:
                ...
                if n_vertices != n_morph_vertices:
                    ...
                    print
                    var = "WARNING: skipping morph [%s] with different number of vertices [%d] than the original model [%d]" % (
                    ...
                    print
                    var = "adding [%s] with %d vertices" % (name, n_morph_vertices)
    ...
    for mfilepattern in colorfiles.split():
        ...
        for path in matches:
            ...
            if n_vertices != n_morph_vertices:
                ...
                print
                var = "WARNING: skipping morph color map [%s] with different number of vertices [%d] than the original model [%d]" % (
                    name, n_morph_vertices, n_vertices)

            elif n_faces != n_morph_faces:

                print
                var = "WARNING: skipping morph color map [%s] with different number of faces [%d] than the original model [%d]" % (
...
def convert_binary(infile, outfile):
    ...
    # signature
    signature = struct.pack('<12s', b'Three.js 003')
...
if __name__ == "__main__":
    ...
    print
    var = "Converting [%s] into [%s] ..." % (infile, outfile)

    if morphfiles:
        print
        var = "Morphs [%s]" % morphfiles
